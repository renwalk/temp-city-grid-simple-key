...
function setup() {
  ...
  var canvasWidth = 1050; // Adjust the canvas width as needed
...
function drawRectangles() {
  ...
  blendMode(BLEND);
  noFill(); // Change the fill color for the sidebar
...
function drawTemperatureKeyAndScale() {
  ...
  noStroke();
  textSize(textScale);
  text("ACTUAL", margin + offset, yPosition - offset + keySize * 0.05);
  ...
  for (var i = 0; i < cityNames.length; i++) {
    ...
    if (col > 0) {
      ...
      line(
        cellX - cellWidth / 2,
        cityKeyY - cellHeight / 2,
        cellX - cellWidth / 2,
        cityKeyY + keySize - cellHeight / 2
      );
    }
    ...
    if (row > 0) {
      line(
        cityKeyX - cellWidth / 2,
        cellY - cellHeight / 2,
        cityKeyX + keySize - cellWidth / 2,
        cellY - cellHeight / 2
      );
    }
  }
